...
function campaignSection(analysis) {
  ...
  const writtenAnalysis = `Posts were followed by an average ${postAverage} tickets by the next scheduled snapshot, compared with ${nonPostAverage} on days without a post. Across ${number.format(analysis.engagementSampleSize || 0)} posts with captured engagement, total engagement has a ${correlationLabel(correlations.total)} correlation with next-day ticket uplift. The relationship by signal is reactions ${correlationLabel(correlations.reactions)}, comments ${correlationLabel(correlations.comments)}, and shares ${correlationLabel(correlations.shares)}. Posts above the median engagement level (${number.format(analysis.medianEngagement || 0)} interactions) averaged +${number.format(analysis.highEngagementAverage || 0)} next-day tickets, compared with +${number.format(analysis.lowEngagementAverage || 0)} for posts at or below that level. This is a useful directional result, not proof of causation: demand and post frequency both increased close to opening.`;
